...
void entfernungsMessung()
{
    ...
    while(itm1 != daten1.end())
    {
        while(itm2 != daten2.end())
        {
            ...
            {
                while(itm1 != daten1.end() && itm2 != daten1.end())
                {
                    std::cout << "Zum Zeitpunkt " << itm1->first << " beträgt die Entfernung: " << distance(itm1->second.lat, itm1->second.lon, itm2->second.lat, itm2->second.lon) << cs1 << "-Koordinaten sind " << itm1->second.lat << " und " << itm1->second.lon << ". " << cs2 << "-Koordinaten sind " << itm2->second.lat << " und " << itm2->second.lon << "."<< std::endl;
                    ++itm1;
                    ++itm2;
            }
        }
